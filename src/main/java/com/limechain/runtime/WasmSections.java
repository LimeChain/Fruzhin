...
public class WasmSections {
    private static final byte[] RUNTIME_VERSION_KEY = "runtime_version".getBytes();
    private static final byte[] RUNTIME_APIS_KEY = "runtime_apis".getBytes();
    private final RuntimeVersion runtimeVersion = new RuntimeVersion();
    ...
    public void processCustomSection(byte[] customSectionContent) {
        ...
        log.log(Level.INFO, "Custom section found in wasm code "
                + Arrays.toString(Arrays.copyOfRange(customSectionContent, 0, 100)) + "...");
