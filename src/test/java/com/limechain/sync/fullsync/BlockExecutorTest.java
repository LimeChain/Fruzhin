...
class BlockExecutorTest {
    ...
    void kusamaFirstBlock() throws IOException {
        ...
        final byte[] genesisStateRoot = StringUtils.hexToBytes("b0006203c3a6e6bd2c6a17b1d4ae8ca49a31da0f4579da950b127774b44aef6b");
        ...
        // Now, prepare the arguments for the actual runtime calls
        // TODO: Encapsulate these more properly
        Block block = getKusamaFirstBlock();
        byte[] encodedUnsealedHeader = ScaleUtils.Encode.encode(BlockHeaderScaleWriter.getInstance()::writeUnsealed, block.getHeader());
        ...
        assertDoesNotThrow(() -> genesisRuntime.call("Core_execute_block", executeBlockParam),
            "Executing the first block of Kusama must not throw any exceptions.");

        // Explicitly assert that we calculated the expected state root
        // TODO: This recalculates the merkle root again... where do we fetch the actual result from... the db... and how?
        byte[] actualRoot = trieAccessor.getMerkleRoot(StateVersion.V0);

        byte[] expectedRoot = block.getHeader().getStateRoot().getBytes();
        assertArrayEquals(expectedRoot, actualRoot, "State root must match the one in the block header.");
    }
